...
import { UPSTASH_DEMO_PREFIX, UPSTASH_WIDGET_PREFIX } from "~/utils/const";
import AppError from "~/utils/error";
import { type ConfigRepository } from "./configRepository";

export class ConfigDemoRepository implements ConfigRepository {
  private redis;
  ...
  async get(id: string): Promise<WidgetConfig> {
    const key = UPSTASH_DEMO_PREFIX + UPSTASH_WIDGET_PREFIX + id;
  ...
  async getAll(): Promise<WidgetConfig[]> {
    const key = UPSTASH_DEMO_PREFIX + UPSTASH_WIDGET_PREFIX;
  ...
  async setAll(data: WidgetConfig[]): Promise<void> {
    ...
    //   const key = UPSTASH_DEMO_PREFIX + UPSTASH_WIDGET_PREFIX + config.id;
    //   try {
    //     await this.redis.set(key, JSON.stringify(data));
    //   } catch (error) {
    //     throw new AppError(
