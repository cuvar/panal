...
function updateTokenInCookie(token: string, cookieName: string, tokenHours: number) {
	var now = new Date();
...
async function verifyPassword(user: string, pwd: string, cookieName: string): Promise<boolean> {
...
export { getTokenFromCookie, digestMessage, updateTokenInCookie, verifyPassword, verifyToken };
